...
class MetadataDialog(tk.Toplevel):
    def __init__(self, parent: tk.Misc, record: DocumentRecord, *, allowed_types: list[str]) -> None:
        ...
        # IMPORTANT: make column 1 expandable for entry widgets
        ...
        self.e_area = ttk.Entry(frm, width=30)
        self.e_area.grid(row=2, column=1, sticky="w", pady=4)
        if record.area:
            self.e_area.insert(0, record.area)

        ttk.Label(frm, text=tr("documents.meta.lbl.process", "Process")).grid(row=3, column=0, sticky="w", pady=4)
        self.e_process = ttk.Entry(frm, width=30)
        self.e_process.grid(row=3, column=1, sticky="w", pady=4)
        if record.process:
            self.e_process.insert(0, record.process)

        # FIX: correct key + defensive next_review handling
        ttk.Label(frm, text=tr("documents.meta.lbl.nextreview", "Next review (YYYY-MM-DD)")).grid(
            row=4, column=0, sticky="w", pady=4
        )
        self.e_next = ttk.Entry(frm, width=20)
        self.e_next.grid(row=4, column=1, sticky="w", pady=4)

        iso = self._normalize_next_review_to_iso(getattr(record, "next_review", None))
        if iso:
            self.e_next.insert(0, iso)

        # Buttons (were missing -> user could not confirm)
        btns = ttk.Frame(frm)
        btns.grid(row=5, column=0, columnspan=2, sticky="e", pady=(12, 0))
        ttk.Button(btns, text=tr("common.cancel", "Cancel"), command=self._cancel).grid(row=0, column=0, padx=(0, 6))
    ...
    @staticmethod
    def _normalize_next_review_to_iso(value: Any) -> Optional[str]:
        """
        Normalize next_review to an ISO date string.
        Accepts: None | str | date | datetime
        """
        if not value:
            return None
    ...
    def _ok(self) -> None:
        ...
        if txt:
            # Accept plain "YYYY-MM-DD" as well as full ISO datetime
            try:
                if len(txt) == 10:
                    d = date.fromisoformat(txt)
                    next_review = datetime(d.year, d.month, d.day)
                else:
