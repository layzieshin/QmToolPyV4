...
class PlacementDialog(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Misc, pdf_path: str,
                 ...
                 *,
                 ...
                 label_color_hex: str = "#000000") -> None:
        ...
        # reduced label UI: position + one offset
        ...
        self._label_name_text = label_name_text or ""
        self._date_format = date_format
        self._label_rgb = _hex_to_rgb(label_color_hex or "#000000")

        # placement in PDF pts
        ...
        ctrl = ttk.Frame(top); ctrl.grid(row=0, column=0, sticky="w")
        ...
        self._page_spin = ttk.Spinbox(ctrl, from_=0, to=len(self._reader.pages)-1,
                                      textvariable=self._page_index, width=5, command=self._on_page_change)
        ...
        w_entry = ttk.Entry(ctrl, textvariable=self._width, width=10)
        w_entry.grid(row=0, column=3, padx=(6, 12), sticky="w")
        w_entry.bind("<FocusOut>", lambda e: self._refresh_canvas())

        ttk.Label(ctrl, text=T("core_signature.name_pos") or "Name position").grid(row=1, column=0, sticky="w", pady=(8, 0))
        cbn = ttk.Combobox(ctrl, textvariable=self._name_pos, values=[v.value for v in LabelPosition], state="readonly", width=14)
        cbn.grid(row=1, column=1, padx=(6, 12), sticky="w"); cbn.bind("<<ComboboxSelected>>", lambda e: self._refresh_canvas())
        ttk.Label(ctrl, text="Name Offset (pt)").grid(row=1, column=2, sticky="w", pady=(8, 0))
        en = ttk.Entry(ctrl, textvariable=self._name_offset, width=10)
        en.grid(row=1, column=3, padx=(6, 12), sticky="w"); en.bind("<FocusOut>", lambda e: self._refresh_canvas())

        ttk.Label(ctrl, text=T("core_signature.date_pos") or "Date position").grid(row=2, column=0, sticky="w", pady=(8, 0))
        cbd = ttk.Combobox(ctrl, textvariable=self._date_pos, values=[v.value for v in LabelPosition], state="readonly", width=14)
        cbd.grid(row=2, column=1, padx=(6, 12), sticky="w"); cbd.bind("<<ComboboxSelected>>", lambda e: self._refresh_canvas())
        ttk.Label(ctrl, text="Date Offset (pt)").grid(row=2, column=2, sticky="w", pady=(8, 0))
        ed = ttk.Entry(ctrl, textvariable=self._date_offset, width=10)
        ed.grid(row=2, column=3, padx=(6, 12), sticky="w"); ed.bind("<FocusOut>", lambda e: self._refresh_canvas())

        self._canvas = tk.Canvas(top, width=self.CANVAS_MAX_W, height=self.CANVAS_MAX_H, bg="#f8f8f8",
                                 highlightthickness=1, highlightbackground="#888")
        self._canvas.grid(row=1, column=0, pady=(10, 8)); self._canvas.bind("<Button-1>", self._on_click)

        btns = ttk.Frame(top); btns.grid(row=2, column=0, sticky="e", pady=(6, 0))
        ttk.Button(btns, text=T("common.cancel") or "Cancel", command=self._cancel).pack(side="right", padx=(6, 0))
        ttk.Button(btns, text=T("core_signature.sign.confirm") or "Place & Sign", command=self._ok).pack(side="right")

        if pdfium is None:
            messagebox.showinfo("Live-Vorschau",
                                "Für die präzise Seitenvorschau bitte 'pypdfium2' installieren:\n\npip install pypdfium2",
                                parent=self)
        self._refresh_canvas(); self._center_if_not_visible()
    ...
    def _refresh_canvas(self):
        ...
        offx = (self.CANVAS_MAX_W - cw) / 2; offy = (self.CANVAS_MAX_H - ch) / 2
        self._offset = (offx, offy)

        # präzise Hintergrundseite
        self._bg_img_tk = None
        if pdfium is not None:
            try:
                pdf = pdfium.PdfDocument(self._pdf_path)
                page = pdf[self._page_index.get()]
                # API-kompatibel: zuerst render_topil, sonst render().to_pil()
                try:
                    pil = page.render_topil(scale=self._scale)  # older helper
                except Exception:
                    bm = page.render(scale=self._scale)         # new API
                    pil = bm.to_pil()
        ...
        if self._bg_img_tk is None:
            # Fallback: weißer Rahmen + Maße
            self._canvas.create_rectangle(offx, offy, offx + cw, offy + ch, fill="white", outline="#666")
            orient = "landscape" if pw > ph else "portrait"
            self._canvas.create_text(offx + 6, offy + 6,
                                     text=f"{int(pw)} x {int(ph)} pt  •  {orient}",
                                     anchor="nw", fill="#666", font=("Segoe UI", 9))

        self._draw_signature_preview(); self._draw_labels_preview()

    def _center_if_not_visible(self):
        pw, ph = self._page_size()
        w = max(12.0, float(self._width.get())); h = w * (self._sig_aspect if self._sig_aspect > 0 else 0.3)
        w = min(w, max(12.0, pw - 2.0)); h = min(h, max(6.0, ph - 2.0))
        x0_cv, y0_cv = self._canvas_from_pdf(self._px, self._py + h)
        x1_cv, y1_cv = self._canvas_from_pdf(self._px + w, self._py)
        in_view = (0 <= x0_cv <= self.CANVAS_MAX_W and 0 <= x1_cv <= self.CANVAS_MAX_W and
                   0 <= y0_cv <= self.CANVAS_MAX_H and 0 <= y1_cv <= self.CANVAS_MAX_H)
        if not in_view:
            self._px = (pw - w) / 2.0; self._py = (ph - h) / 2.0; self._refresh_canvas()
    ...
    def _pdf_from_canvas(self, x_cv: float, y_cv: float) -> tuple[float, float]:
        offx, offy = self._offset; pw, ph = self._page_size()
    ...
    def _draw_signature_preview(self):
        ...
        if self._sig_png:
            try:
                ...
                # ~60% alpha
                r, g, b, a = sig.split(); a = ImageEnhance.Brightness(a).enhance(0.6)
    ...
    def _draw_labels_preview(self):
        ...
        if self._show_name and self._label_name_text:
            ...
            self._canvas.create_text(nx, ny, text=self._label_name_text, anchor="sw", fill=col,
        ...
        if self._show_date:
            ...
            self._canvas.create_text(dx, dy, text=date_text, anchor="sw", fill=col,
                                     font=("Segoe UI", 10, "bold"))
    ...
    def _on_click(self, e):
        ...
        w_pt = max(12.0, float(self._width.get()))
        h_pt = w_pt * (self._sig_aspect if self._sig_aspect > 0 else 0.3)
        x_pt = max(0.0, min(x_pt, pw - w_pt)); y_pt = max(0.0, min(y_pt, ph - h_pt))
        self._px, self._py = x_pt, y_pt; self._refresh_canvas()
    ...
    def _ok(self):
        ...
        self.result = (placement, (name_pos, date_pos), offsets); self.destroy()
