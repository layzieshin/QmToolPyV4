...
class SignatureService:
    def __init__(self, *, settings_manager: SettingsManager, logger: Optional[Any]=None,
        ...
        if naming_registry: self._strategies.update(naming_registry)
    ...
    def load_config(self) -> SignatureConfig:
        ...
        def get_user(k: str, d):
            if uid: return self._sm.get(_FEATURE_ID, k, d, user_specific=True, user_id=uid)
            return self._sm.get(_FEATURE_ID, k, d)

        def get_global(k: str, d): return self._sm.get(_FEATURE_ID, k, d)
    ...
    def save_config(self, cfg: SignatureConfig) -> None:
        user = getattr(AppContext, "current_user", None)
        uid = getattr(user, "id", None)
        def set_user(k: str, v: Any):
            if uid: self._sm.set(_FEATURE_ID, k, v, user_specific=True, user_id=uid)
            else:   self._sm.set(_FEATURE_ID, k, v)
    ...
    def is_password_required(self) -> bool:
        ...
        if cfg.admin_password_policy == AdminPasswordPolicy.ALWAYS: return True
        if cfg.admin_password_policy == AdminPasswordPolicy.NEVER: return False
        return bool(cfg.user_pwd_required)

    def verify_password(self, user_id: str, password: str) -> bool:
        if callable(self._password_verifier):
            try:
                if bool(self._password_verifier(user_id, password)): return True
            ...
            if auth and hasattr(auth, "verify_password"):
                ...
                for args in ((user_id, password), (uname, password) if uname else None, (password,)):
                    ...
                    try:
                        ...
                        try:
                            if bool(fn(user_id=user_id, password=password)): return True
                        except Exception:
                            try:
                                if uname and bool(fn(username=uname, password=password)): return True
                            except Exception: pass
        except Exception: pass
        try:
            if hasattr(AppContext, "verify_password"):
                fn2 = getattr(AppContext, "verify_password")
                for args in ((user_id, password), (uname, password) if uname else None, (password,)):
                    if args is None: continue
                    try:
                        if bool(fn2(*args)): return True
                    except TypeError:
                        try:
                            if bool(fn2(user_id=user_id, password=password)): return True
                        except Exception:
                            try:
                                if uname and bool(fn2(username=uname, password=password)): return True
                            except Exception: pass
        except Exception: pass
        try:
            ...
            if bool(bridge_verify(user_id=int(user_id) if user_id is not None else None,
                                  username=uname, password=password)): return True
        except Exception: pass
        return False
    ...
    def render_png_from_strokes(self, strokes: list[list[tuple[int,int]]],
        ...
        # (ab jetzt bei Capture NICHT mehr genutzt, bleibt kompatibel)
        if name_text:
            y = 4 if name_pos == LabelPosition.ABOVE else h - 14
            drw.text((4, y), name_text, fill=(0,0,0,255))
        if date_text:
            y = 20 if date_pos == LabelPosition.ABOVE else h - 30
            drw.text((4, y), date_text, fill=(0,0,0,255))
        buf = io.BytesIO(); img.save(buf, format="PNG"); return buf.getvalue()

    # ------------------------------ Signing ----------------------------------
    def _hash_png(self, data: bytes) -> str:
        ...
        try:
            if self._logger and hasattr(self._logger, "log"):
                self._logger.log(feature="Signature", event="sign_pdf",
                                 reference=payload.get("input_path"), data=payload); return
        except Exception: pass
    ...
    def sign_pdf(self, input_path: str, placement: SignaturePlacement, *,
                 ...
                 override_label_offsets: Optional[LabelOffsets]=None) -> str:
        user = getattr(AppContext, "current_user", None)
        uid = getattr(user, "id", None)
        display_name = (getattr(user, "full_name", None) or getattr(user, "name", None) or getattr(user, "username", None))
        if not uid: raise RuntimeError("No current user; signing requires authentication.")

        cfg = self.load_config()
        sig: Optional[bytes] = None
        if use_user_signature:
            sig = self.load_user_signature_png(uid)
            if not sig and not raw_signature_png: raise RuntimeError("No stored signature for user.")
        if raw_signature_png: sig = raw_signature_png
        if sig is None: raise RuntimeError("Signature image not available.")

        name_pos, date_pos = cfg.name_position, cfg.date_position
        if enforce_label_positions: name_pos, date_pos = enforce_label_positions
        ...
        labels = RenderLabels(
            ...
            name_pos=name_pos, date_pos=date_pos, date_format=cfg.date_format,
        ...
        if override_output:
            ...
            if cfg.naming_mode == OutputNamingMode.EXTERNAL_STRATEGY and cfg.external_strategy_id:
                ...
                if not strat: raise RuntimeError(f"Unknown naming strategy '{cfg.external_strategy_id}'.")
            else:
        ...
        self._audit({
            ...
            "name_position": name_pos.value, "date_position": date_pos.value, "date_format": cfg.date_format,
            "label_color": cfg.label_color,
            "naming_mode": cfg.naming_mode.value, "reason": reason,
            "signature_sha256": self._hash_png(sig)
        })
        return out_path
