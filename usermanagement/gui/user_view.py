...
class UserManagementView(Frame):
    ...
    def _get_selected_username(self) -> Optional[str]:
        """Safely get username from selected tree item."""
        sel = self. admin_user_tree.focus()
        if not sel:
            return None

        values = self.admin_user_tree.item(sel).get("values")
        if not values:
            return None

        # Treeview may return int instead of str - always convert
    ...
    def _popup_edit_user(self):
        ...
        if not user:
            self.set_status(T("core.update_failed"))
            messagebox.showerror(T("core.error"), f"User not found: {username}")
            return
        ...
        for i, f in enumerate(fields):
            ...
            if f == "role":
                ...
                e = Entry(pop); e.insert(0, getattr(user, f, "") or ""); e.grid(row=i, column=1, pady=2, padx=4)
                ents[f] = e
